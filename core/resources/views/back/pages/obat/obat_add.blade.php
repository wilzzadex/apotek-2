                                ...
                                <div class="row">
                                    <div class="col-12" id="renderSatuan">
                                        <div class="form-group row jml_satuan">
                                            ...
                                            <div class="col-3">
                                                <label id="label-harga-beli">+</label>
                                                <input type="text" name="harga_beli[]" required id="harga-beli-1" class="form-control">
                                            </div>
                                        </div>
                                        <br>
...
<script>
    ...
    let el_id = 1; 
    let val_harga_beli;
    let lHargabli = $('#satuan_id_1').find(':selected').text();
    $('#label-harga-beli').html('Harga Beli Per ' + lHargabli + ' (Rp)');

    $('#harga-beli-1').mask('000.000.000', { reverse: true });

    $('#satuan_id_1').on('change', function(){
        lHargabli = $(this).find(':selected').text();
        $('#label-harga-beli').html('Harga Beli Per ' + lHargabli + ' (Rp)');
        console.log(el_id);
    })
    ...
    $('#btn-satuan').on('click',function(){
        ...
        if($('#harga-beli-'+el_id).val() == ''){
            customAlert('', 'Harga beli tidak boleh kosong !', 'warning');
        }else{
            if(elemen_satuan <= 3){
                el_id = (parseInt(elemen_satuan) + 1);
                var val1=[]; 
                ...
                $.ajax({
                    ...
                    success : function(res){
                        ...
                        customAlert('Tingkat satuan sudah maksimal !','','warning');
                            ...
                            html_satuan += `<div class="form-group row jml_satuan" id="id_div_`+el_id+`">
                                            <div class="col-3">
                                            ...
                                            <div class="col-3">
                                                ...
                                                <input type="number" data-myid="`+el_id+`" min="1" value="0" name="jumlah[]" id="jumlah-obat-`+el_id+`" onkeyup="hitungHarga(this)" required class="form-control" placeholder="Masukan Jumlah" aria-describedby="basic-addon2"/>
                                                <input type="hidden" name="sama_dengan[]" value="`+res.satuan_next.id+`">
                                            </div>
                                            <div class="col-3">
                                                <label id="label-harga-beli-`+el_id+`"></label>
                                                    <input type="text" name="harga_beli[]" required id="harga-beli-`+el_id+`" class="form-control">
                                            </div>
                                            <div class="col-1">
                                                <button type="button" onclick="deleteDiv(`+el_id+`)" class="btn btn-sm btn-danger" style="margin-top:30px"><i class="fa fa-trash"></i></button>
                                            </div>
                        ...
                        let prev_el_id = parseInt(el_id) -1;
                        let prev_harga_beli = $('#harga-beli-'+prev_el_id).val(); 
                        $('#harga-beli-'+prev_el_id).prop('readonly',true); 
                        val_harga_beli = prev_harga_beli.replace(/\./g, "");
                        console.log(val_harga_beli);
                        let lHargabli2 = $('#satuan_id_'+el_id).find(':selected').text();
                        $('#label-harga-beli-'+el_id).html('Harga Beli Per ' + lHargabli2 + ' (Rp)');
                        $('#harga-beli-'+el_id).mask('000.000.000', { reverse: true });
    ...
    function hitungHarga(thiss){
        let myId = $(thiss).attr('data-myid');
        let prev_el_id = parseInt(myId) -1;
        let prev_harga_beli = $('#harga-beli-'+prev_el_id).val();
        let prev_val_harga_beli = prev_harga_beli.replace(/\./g, "");
        let jumlah_val = $(thiss).val();
        let hargas = parseInt(jumlah_val) * parseInt(prev_val_harga_beli);
        $('#harga-beli-'+myId).mask('000.000.000', { reverse: true }).val(hargas).trigger('input');
    }

    function deleteDiv(id){
        $('#id_div_'+el_id).remove();
        el_id = parseInt(el_id) - 1;
        $('#harga-beli-'+el_id).removeAttr('readonly'); 
    }
